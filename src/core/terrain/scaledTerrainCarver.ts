...
/**
 * Mathematically scale-invariant terrain feature carver (caves, tunnels, arches, islands).
 * Performs exact PRNG step reproduction scaled down to preview target resolution.
 */
export function carveScaledTerrainFeatures(
  grid: Uint8Array,
  prng: SeededRandom,
  theme: MapTheme,
  previewW: number,
  ...
  if (tunnels > 0) {
    for (let t = 0; t < tunnels; t++) {
      ...
      for (let s = 0; s < steps; s++) {
        ...
        if (tx < 60 * scaleX || tx > previewW - 60 * scaleX || ty < 35 * scaleY || ty > waterLevel - 35 * scaleY) {
          angle += Math.PI * 0.5 + (prng.next() - 0.5) * 0.3;
          tx = Math.max(65 * scaleX, Math.min(previewW - 65 * scaleX, tx));
          ty = Math.max(40 * scaleY, Math.min(waterLevel - 40 * scaleY, ty));
        }

        const minX = Math.max(0, Math.floor(tx - tunnelRadius));
        const maxX = Math.min(previewW - 1, Math.ceil(tx + tunnelRadius));
        const minY = Math.max(hasSolidCeiling ? Math.round(17 * scaleY) : 0, Math.floor(ty - tunnelRadius));
        const maxY = Math.min(previewH - 1, Math.ceil(ty + tunnelRadius));
        ...
        for (let y = minY; y <= maxY; y++) {
          ...
          const endX = Math.min(previewW - 1, Math.floor(tx + dxMax));
          if (startX <= endX) {
            grid.fill(0, y * previewW + startX, y * previewW + endX + 1);
          }
        }
      }
    }
    ...
    for (let i = 0; i < chamberCount; i++) {
      ...
      const maxY = Math.min(previewH - 1, Math.ceil(cy + ry));
      ...
      for (let y = minY; y <= maxY; y++) {
        ...
        const endX = Math.min(previewW - 1, Math.floor(cx + dxMax));
        if (startX <= endX) grid.fill(0, y * previewW + startX, y * previewW + endX + 1);
      }
    }
  }
  ...
  // 2. Rock Arches
  if (arches > 0) {
    const archPositions = [previewW * 0.32, previewW * 0.68];
    for (const archX of archPositions) {
      const archY = previewH * 0.54;
      const rx = 120 * scaleX;
      const ry = 90 * scaleY;
      const rySq = ry * ry;
      const rxOverRy = rx / ry;
      const minY = Math.max(0, Math.floor(archY - ry));
      const maxY = Math.min(previewH - 1, Math.ceil(archY + ry + 30 * scaleY));
      ...
      for (let y = minY; y <= maxY; y++) {
        ...
        const endX = Math.min(previewW - 1, Math.floor(archX + dxMax));
        if (startX <= endX) grid.fill(0, y * previewW + startX, y * previewW + endX + 1);
      }
    }
  }
  ...
  if (diggers > 0) {
    for (let w = 0; w < diggers; w++) {
      ...
      for (let s = 0; s < steps; s++) {
        ...
        if (wx < 60 * scaleX || wx > previewW - 60 * scaleX || wy < 32 * scaleY || wy > waterLevel - 35 * scaleY) {
          angle += Math.PI * 0.5 + (prng.next() - 0.5) * 0.3;
          wx = Math.max(65 * scaleX, Math.min(previewW - 65 * scaleX, wx));
        ...
        const maxY = Math.min(previewH - 1, Math.ceil(wy + diggerRadius));
        ...
        for (let y = minY; y <= maxY; y++) {
          ...
          const endX = Math.min(previewW - 1, Math.floor(wx + dxMax));
          if (startX <= endX) grid.fill(0, y * previewW + startX, y * previewW + endX + 1);
        }
      }
    }
    ...
    for (let h = 0; h < hubCount; h++) {
      ...
      const maxY = Math.min(previewH - 1, Math.ceil(hy + hr));
      ...
      for (let y = minY; y <= maxY; y++) {
        ...
        const endX = Math.min(previewW - 1, Math.floor(hx + dxMax));
        if (startX <= endX) grid.fill(0, y * previewW + startX, y * previewW + endX + 1);
      }
    }
  }
  ...
  const stampTacticalFloatingIsland = (fx: number, fy: number, rx: number, ry: number) => {
    ...
    const maxX = Math.min(previewW - 1, Math.ceil(fx + rx));
