...
export const WeaponGridCard: React.FC<WeaponGridCardProps> = ({
  ...
  return (
    <button
      disabled={isDisabled}
      onClick={onSelect}
      className={`p-2.5 rounded-2xl border text-left flex flex-col justify-between min-h-[128px] h-full transition-all relative overflow-hidden group ${
        isSelected
    ...
    >
      {isSelected && (
        <div className="absolute top-2 right-2 p-0.5 bg-violet-600 border border-violet-300 text-white rounded-full shadow z-10 animate-in zoom-in-50 duration-150">
          <Check className="w-2.5 h-2.5" />
        </div>
      )}
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <div
            className={`w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-zinc-950/90 border border-zinc-800 flex items-center justify-center text-xl shadow-inner shrink-0 ${
          ...
          {isLocked ? (
            <div className="text-[9px] font-black px-1.5 py-0.5 rounded-lg border bg-amber-950/80 border-amber-500/60 text-amber-300 flex items-center gap-0.5 shadow-sm">
              <Lock className="w-2.5 h-2.5 text-amber-400" /> Tour {turnDelay}
            </div>
          ) : (
            <div
              className={`text-[10px] font-black px-1.5 py-0.5 rounded-lg border ${
                ammo === -1
        ...
        <div className="flex items-center gap-1 mb-0.5">
          ...
          {isSuperWeapon && (
            <span className="text-[7.5px] font-black uppercase bg-purple-950 text-purple-300 border border-purple-500/40 px-1 py-0.2 rounded-full shrink-0">
              Super ⭐
            </span>
          )}
        </div>

        <p className="text-[10px] text-zinc-400 line-clamp-2 leading-tight mb-1">
          {weapon.description}
        </p>
      </div>

      <div className="pt-1 border-t border-zinc-800/80 flex items-center justify-between text-[9.5px] font-medium text-zinc-400 shrink-0">
        {isLocked ? (
          <div className="text-amber-400 font-bold flex items-center gap-0.5">
            <Lock className="w-2.5 h-2.5 text-amber-400" />
            <span>Tour {turnDelay} ({roundsRemaining} rest.)</span>
          </div>
        ) : (
          <>
            <div className="flex items-center gap-0.5 text-red-400 font-bold">
              <Zap className="w-2.5 h-2.5 text-red-400" />
              <span>{weapon.damage} Dgt</span>
            </div>
            <div>
              {weapon.radius > 0 ? (
                <span className="text-zinc-400">Rayon: {weapon.radius}px</span>
              ) : (
                <span className="text-zinc-500 italic">Direct</span>
              )}
            </div>
          </>
        )}
