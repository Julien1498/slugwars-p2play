  ...
  return (
    <div className="flex flex-col h-screen max-h-screen overflow-hidden bg-zinc-950 p-1 md:p-1.5 text-zinc-100 relative">
      ...
      <div className="relative flex-1 min-h-0 flex flex-col items-center justify-center overflow-hidden my-0.5">
        ...
        {/* Tactical Fuse Timer Floating Widget - Positioned in the bottom center of the canvas */}
        {activeWeapon?.allowCustomFuse && (gameState.phase === 'AIMING' || gameState.phase === 'TURN_TIME') && (
            ...
            <div className="flex items-center gap-1.5">
              {[1, 2, 3, 4, 5].map((sec) => {
                ...
                return (
                  ...
                  >
                    <span>{sec}s</span>
                    <span className={`text-[8px] leading-none ${isSelected ? 'text-zinc-950 font-bold' : 'text-zinc-500'}`}>[{sec}]</span>
                  </button>
                );
              })}
            </div>
